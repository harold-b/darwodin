...
RotationGestureRecognizer_VTable :: struct {
    ...
    velocity: proc(self: ^RotationGestureRecognizer) -> CG.Float,
    load: proc(),
    initialize: proc(),
...
RotationGestureRecognizer_odin_extend :: proc(cls: Class, vt: ^RotationGestureRecognizer_VTable) {
    ...
    if vt.velocity != nil {
        velocity :: proc "c" (self: ^RotationGestureRecognizer, _: SEL) -> CG.Float {

            vt_ctx := ObjC.object_get_vtable_info(self)
            context = vt_ctx._context
            return (cast(^RotationGestureRecognizer_VTable)vt_ctx.super_vt).velocity(self)
        }
